...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE conversations ADD CONSTRAINT conversations_participant_1_id_participant_2_id_key UNIQUE(participant1_id, participant2_id);
  END IF;
END $$;
...
CREATE POLICY "Users can read own conversations"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p
      WHERE (p.id = participant1_id OR p.id = participant2_id) AND p.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Users can read conversation messages"
  ...
  USING (
    EXISTS (
      ...
      JOIN profiles p ON (p.id = c.participant1_id OR p.id = c.participant2_id)
