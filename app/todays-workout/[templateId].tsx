    ...
    try {
      ...
      if (template) {
        ...
        const exercisesWithDetails: ExerciseWithDetails[] = template.exercises.map((templateExercise: TemplateExercise, index: number) => {
          ...
          // Add null check for templateExercise.exercise
          if (!templateExercise.exercise) {
            console.warn('Template exercise has null exercise data:', templateExercise);
            return null;
          }
          ...
          const exerciseData = templateExercise.exercise || {};
          const { section_title: _omit, ...exerciseRest } = exerciseData;
          ...
          const mappedExercise = {
            ...
            image: getExerciseImageUtil(exerciseData.name || 'Unknown Exercise', index),
            video_url: exerciseData.video_url || 'https://www.w3schools.com/html/mov_bbb.mp4',
            difficulty: (exerciseData.difficulty_level || 'intermediate').toLowerCase(),
            alternatives: getExerciseAlternatives(exerciseData),
            muscleGroups: Array.isArray(exerciseData.muscle_groups) && exerciseData.muscle_groups.length > 0
              ? exerciseData.muscle_groups
              : getMuscleGroups(exerciseData.category || 'general'),
            equipmentList: Array.isArray(exerciseData.equipment)
              ? exerciseData.equipment
              : (typeof exerciseData.equipment === 'string' && exerciseData.equipment)
                ? [exerciseData.equipment]
                : getRequiredEquipment(exerciseData.name || 'Unknown Exercise'),
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.heroSection, { height: heroHeightAnim }]}> 
        ...
        <Image 
          source={{ uri:  workout?.thumbnail_url ||'https://images.pexels.com/photos/1552242/pexels-photo-1552242.jpeg?auto=compress&cs=tinysrgb&w=800' }}
